...
-(SocketIOClient *)client{
    ...
    if (!_client) {
         _client = [[SocketIOClient alloc] initWithSocketURL:[NSURL URLWithString:@"http://103.231.167.85:9081"] config:@{@"log": @YES, @"forcePolling": @YES}];
...
-(void)connection{
    ...
    [self.client on:@"connect" callback:^(NSArray* data, SocketAckEmitter* ack) {
        ...
        [self.client emit:@"joingame" with:@[str]];
    }];
    //链接成功
    [self.client on:@"connect" callback:^(NSArray* data, SocketAckEmitter* ack) {
        NSLog(@"*************\n\niOS客户端上线\n\n************");
...
//    }];
    ...
    [self.client on:@"hall" callback:^(NSArray * _Nonnull data, SocketAckEmitter * _Nonnull emit) {
